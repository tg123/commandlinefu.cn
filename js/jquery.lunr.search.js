  ...
  var LunrSearch = (function() {
    ...
    LunrSearch.prototype.initialize = function() {
      ...
      this.loadIndexData(function(data) {
        ...
        var input = self.$elem;
        input.blur(function(){ result.hide('slow');  });
        input.focus(function(){ if(input.val()) self.search(input.val()); });
    ...
    LunrSearch.prototype.displayResults = function(entries) {
      ...
      if (entries.length === 0) {
        $entries.append('<p>command not found</p>')
      } else {
        $entries.append(this.template({entries: entries.slice(0, 10)}));
      }
